...
public abstract class Canvas extends Displayable {
   ...
   void callPaint(Graphics g) {
      boolean tickerOnly = false;
      super.callPaint(g);
      if (this.ticker != null) {
         synchronized(Display.LCDUILock) {
            if (!this.fullScreenMode && this.ticker != null) {
               if (g.getClipY() + g.getClipHeight() < canvasTickMainZone.y) {
                  tickerOnly = true;
               } else {
                  g.translate(canvasTickMainZone.x, canvasTickMainZone.y);
                  g.clipRect(0, 0, canvasTickMainZone.width, canvasTickMainZone.height);
               }
            }
         }
      }
   ...
   static {
      ...
      canvasTickMainZone = uistyle.getZone(45);
      canvasTickTickerZone = uistyle.getZone(46);
   }
}
