...
public abstract class Item {
   ...
   int getEmptyStringHeight(int width, Font font) {
      ...
      TextBreaker breaker = TextBreaker.getBreaker(this.emptyStringFont.getImpl(), EMPTY_STRING, false);
   ...
   boolean boundsIncludeOtherItems(int rowHeight, int pW, int viewableWidth) {
   ...
   static String getStringWithSeparator(String string) {
      ...
      if (string != null) {
         ...
         if (replaceStringSeparator) {
            stringStringBuffer.setCharAt(stringSeparatorInsertionIndex, ITEM_STRING_SEPARATOR);
         } else {
            stringStringBuffer.insert(stringSeparatorInsertionIndex, ITEM_STRING_SEPARATOR);
         }
   ...
   class Label {
      ...
      void defineLabelTextLines(int widthForLabel) {
         ...
         breaker.setFont(Displayable.uistyle.getZone(20).getFont());
      ...
      int paintLabel(Graphics g) {
         if (Item.isStringEmpty(this.labelStrNL)) {
            ...
            ng.drawTextInArea(UIStyle.isAlignedLeftToRight ? tX : tX + Item.this.bounds[2] - this.labelWidth, Item.this.itemIndex > 0 ? tY + UIStyle.LABEL_PAD_BEFORE : tY, this.labelWidth, this.labelHeight, this.textLines, UIStyle.isAlignedLeftToRight ? 1 : 3);
            g.translate(0, this.getLabelHeight());
            colorCtrl.setFgColor(oldFgColor);
            return 0;
         }
      }
