...
public class TextBreaker {
   ...
   public static TextLine breakOneLineTextInArea(int width, int height, Font font, String text, boolean truncate) {
      return breakOneLineTextInArea(width, height, font, text, DEFAULT_TEXT_LEADING, false, truncate);
   }

   public static TextLine breakOneLineTextInArea(int width, int height, Font font, String text, int leading, boolean enableWordWrapping, boolean truncate) {
      if (null != text && !text.equals("")) {
         TextBreaker breaker = null;
         ...
         breaker.setTruncation(truncate);
         TextLine tLine = breaker.getTextLine(width);
         breaker.destroyBreaker();
   ...
   public static TextLine breakOneLineTextInZone(Zone textZone, String text) {
      return breakOneLineTextInArea(32767, textZone.height, textZone.getFont(), text, DEFAULT_TEXT_LEADING, false, false);
   }
   ...
   public static TextLine breakOneLineTextInZone(Zone textZone, boolean useWidth, boolean useHeight, String text, int leading, boolean enableWordWrapping) {
      ...
      tLine = breakOneLineTextInArea(w, h, textZone.getFont(), text, leading, enableWordWrapping, false);
      return tLine;
   }
   ...
   public synchronized TextLine getTextLine(int availableWidth) {
      return this.getTextLine(availableWidth, false);
   }

   public synchronized TextLine getTextLine(int availableWidth, boolean bForceWordWrap) {
      TextLine tLine = null;
      if (this._current_breaker_offset < this.strText.length()) {
         int numberOfChars = this.getLine(availableWidth, this.textParams, this._current_breaker_offset, bForceWordWrap);
         if (numberOfChars != 0) {
