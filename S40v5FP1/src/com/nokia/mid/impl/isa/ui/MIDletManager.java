...
final class MIDletManager implements EventConsumer, ExitManager, MIDletAccess, Runnable {
   ...
   public static void main(String[] args) {
      if (!mainCalled) {
         ...
         if (args == null) {
            ...
            midletPushed = true;
            midletToLaunchIdx = loadMIDletSuite(args[0]);
         } else {
   ...
   private static int loadMIDletSuite(String midletToStartPropStr) {
      ...
      if (midletToStartPropStr != null) {
         for(; !exit; ++_midletCount) {
            try {
               ...
               appProperties = s_getAppProperty(key);
      ...
      while(true) {
         try {
            ...
            String localisedAppProperties = s_getAppProperty(key);
            key = "MIDlet-" + _midletCount;
            appProperties = s_getAppProperty(key);
   ...
   private static void s_loadMIDletExplorer() {
      try {
         Class _meCls = Class.forName("com.nokia.mid.impl.isa.ui.MIDletExplorer");
         s_mExplorerInfo = new MIDletRTInfo(_meCls);
   ...
   static native void notifyStartOrExitMIDlet(boolean var0, int var1, String var2);
   ...
   private void init(int midletToStartIdx) {
      if (s_isSingleMIDlet()) {
         ...
         if (midletNumber > 0 && midletNumber <= s_mInfoList.size()) {
            MIDletRTInfo _mInfo = (MIDletRTInfo)s_mInfoList.elementAt(midletNumber - 1);
            this.launchMIDlet(_mInfo);
         } else {
   ...
   public final void consumeEvent(int category, int type, int param) {
      if (category == 3) {
         ...
         case 1:
            ...
            this.destroyMIDletSuite(_time_out);
         case 2:
         default:
            break;
         case 3:
   ...
   private void constructMIDlet(MIDletRTInfo mInfo) {
      try {
         synchronized(this.mStateMutex) {
            ...
            notifyStartOrExitMIDlet(true, s_mInfoList.indexOf(mInfo) + notChapi, mInfo.getCls().getName());
   ...
   private void moveMIDletExplorerToForeground() {
      if (this.mRunning != null) {
         MIDlet _mE = this.mExplorer.getMIDlet();
         displayAccessor = displayAccessor.replaceDisplay(_mE);
         ((MIDletExplorer)_mE).displayMIDletList();
         this.mRunning = null;
      }
   ...
   private final void destroyMIDletSuite(Throwable t) {
      this.notifyExitBeginError(t);
      this.destroyMIDletSuite(1500);
   }

   private final void destroyMIDletSuite(int time_out) {
      if (this.mRunning != null) {
         try {
   ...
   private void moveMIDletToDestroyed(MIDletRTInfo mInfo) {
      ...
      if (PriAccess.getInt(5) == 1 && this.isExitURISet0() && !s_isSingleMIDlet() && midletNumber == 0) {
   ...
   private void setMIDletToDestroyedAndRemoveInstance(MIDletRTInfo mInfo) {
      ...
      if (_ms != null) {
         ...
         if (!s_isExitInProgress) {
            notifyStartOrExitMIDlet(false, s_mInfoList.indexOf(mInfo) + notChapi, (String)null);
         }
      }
   ...
   private MIDletState getMIDletState(MIDlet m) {
      if (m != null) {
         MIDletState _msPretender = s_mExplorerInfo.getInstance();
         if (_msPretender != null && m == _msPretender.getMIDlet()) {
            ...
            for(int _mIdx = s_mInfoList.size() - 1; _mIdx >= 0; --_mIdx) {
               ...
               _msPretender = _mInfo.getInstance();
               if (_msPretender != null && m == _msPretender.getMIDlet()) {
                  return _msPretender;
               }
            }
   ...
   public final String getMIDletName(MIDlet m) {
      ...
      if (name != null && !name.equals("")) {
         ...
         name = s_getAppProperty("Nokia-MIDlet-Name-" + System.getProperty("microedition.locale"));
         if (name == null) {
            name = s_getAppProperty("MIDlet-Name");
         }
