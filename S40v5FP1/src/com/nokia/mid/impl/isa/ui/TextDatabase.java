...
public final class TextDatabase {
   ...
   public static final int qtn_key_play = 74;
   public static final int qtn_key_previous = 75;
   public static final int qtn_key_next = 76;
   public static final int qtn_key_stop = 77;
   public static final int qtn_form_list_day_monthview = 78;
   public static final int text_kjava_last = 79;
   static final char PLACEHOLDER_BEGIN = '%';
   static final char PLACEHOLDER_STRING_END = 'U';
   static final char PLACEHOLDER_NUM_END = 'N';
   static String[] usedStrings = new String[79];
   static String lastUsedLocale = System.getProperty("microedition.locale");

   public static String getText(int textID) {
      String[] localUsedStrings = usedStrings;

      try {
         if (localUsedStrings[textID] == null) {
            localUsedStrings[textID] = getNativeText(textID);
         }
      } catch (Exception var3) {
         throw new IllegalArgumentException("TextDatabase: " + var3);
      }

      return localUsedStrings[textID];
   }

   public static String getText(int textID, String param) {
      return s_rePlaceHolder(getText(textID), param);
   }

   public static String getText(int textID, String param1, String param2) {
      return s_rePlaceHolder(s_rePlaceHolder(getText(textID), param1), param2);
   }

   private static native String getNativeText(int var0);

   private static String s_rePlaceHolder(String msg, String param) {
      int indexOfBegin = msg.indexOf(37);
      int indexOfU = msg.indexOf(85) + 1;
      int indexOfN = msg.indexOf(78) + 1;
      int indexOfEnd = 0;
      if (indexOfBegin < 0) {
         throw new IllegalArgumentException("TextDatabase: invalid placeholder sequence");
      } else {
         if (indexOfU > 0 && indexOfN > 0) {
            indexOfEnd = indexOfU > indexOfN ? indexOfN : indexOfU;
         } else if (indexOfN > 0) {
            indexOfEnd = indexOfN;
         } else if (indexOfU > 0) {
            indexOfEnd = indexOfU;
         } else if (indexOfU < 0 && indexOfN < 0) {
            throw new IllegalArgumentException("TextDatabase: invalid placeholder sequence");
         }

         return msg.substring(0, indexOfBegin) + param + msg.substring(indexOfEnd);
      }
   }
   ...
   static {
      UIStyle.registerReinitialiseListener(new ReinitialiseListener() {
         public void reinitialiseForForeground() {
            ...
            if (!currentLocale.equals(TextDatabase.lastUsedLocale)) {
               TextDatabase.lastUsedLocale = currentLocale;
               TextDatabase.usedStrings = new String[79];
            }
