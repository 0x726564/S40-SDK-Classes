...
public class Graphics {
      ...
      synchronized(textLines) {
         if (textLines.size() > 0) {
            for(Enumeration tlEnumerator = textLines.elements(); tlEnumerator.hasMoreElements(); offsetY += tLine.getTextLineHeight()) {
               ...
               this.drawTextLine(x, offsetY, width, tLine, alignment);
            }
         }
   ...
   public void drawTextLine(Zone textZone, int offsetX, int offsetY, TextLine textLine, int alignment) {
      this.drawTextLine(textZone.x + offsetX, textZone.y + offsetY, textZone.width, textLine, alignment);
   }

   public void drawTextLine(int x, int y, int width, TextLine textLine, int alignment) {
      int offsetX = x;
      int lineWidth = false;
      if (null != textLine) {
         ...
         if (0 < lineWidth) {
            ...
            this.drawText(textLine, offsetX, y, lineWidth);
         }
      }
      ...
      if (0 != (this.colorCtrl.getFgColor() >>> 24 & 255)) {
         if (tline != null) {
            ...
            if (tWidth + x > 0 && x < zoneWidth) {
               this.renderText(tline, x, top + tline.getTextLineLeading(), segmentAreaWidth);
            }
         }
