...
public class DateInputField {
   ...
   public int render(int x, int y, Graphics ng, int heightOfArea) {
      ...
      if (this.initialized && this.fieldType == 6) {
         this.renderAM_PM(x, y, baseline, ng, cc, heightOfArea);
         return 0;
      } else {
         ...
         if (this.focused) {
            ...
            for(i = cnt; i < this.fieldDigits.length; ++i) {
               int spaceWidth;
               if (this.initialized) {
                  highlight_width += nf.getCharWidth(this.fieldDigits[i]);
                  spaceWidth = nf.getCharASpace(this.fieldDigits[i]) + nf.getCharCSpace(this.fieldDigits[i]);
                  if (spaceWidth > 0) {
         ...
         for(i = cnt; i < this.fieldDigits.length; ++i) {
            if (this.initialized) {
               left = ng.drawUnicode(this.fieldDigits[i], (short)left, (short)baseline);
            } else {
      ...
      if (digits != null) {
         if (this.fieldType == 6) {
            ...
            for(int cnt = digits.length - 1; cnt >= 0; --cnt) {
               fieldValue += (digits[cnt] - 48) * lValue;
               lValue *= 10;
            }
         }
      }
   ...
   private void renderAM_PM(int x, int y, int baseline, Graphics ng, ColorCtrl cc, int heightOfArea) {
      TextLine textLine = this.fieldValue == 0 ? PM_TEXT : AM_TEXT;
      if (this.focused) {
   ...
   static {
      PM_TEXT = TextBreaker.breakOneLineTextInZone(uistyle.getZone(50), true, true, TextDatabase.getText(39), 0, false);
      AM_TEXT = TextBreaker.breakOneLineTextInZone(uistyle.getZone(50), true, true, TextDatabase.getText(40), 0, false);
   }
}
