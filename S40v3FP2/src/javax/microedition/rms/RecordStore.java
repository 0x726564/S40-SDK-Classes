...
public class RecordStore {
   ...
   private static Vector dbCache = (Vector)SharedObjects.get("dbCache", "java.util.Vector");
   private static final Object dbCacheLock = SharedObjects.get("dbCacheLock", "java.lang.Object");
         ...
         if (var0.length() <= 32 && var0.length() != 0) {
            ...
            for(int var5 = 0; var5 < dbCache.size(); ++var5) {
               ...
               if (var4.uniqueIdPath.equals(var2)) {
                  if (var4.recordStoreMidletId == var4.dbraf.getCurrentMidletId()) {
                     ++var4.opencount;
                     return var4;
                  }
   ...
   private void freeRecord(RecordStore.RecordHeader var1) throws RecordStoreException {
      try {
         if (var1.offset == this.dbFirstRecordOffset) {
            ...
            for(var3 = var1; var3.nextOffset != 0 && var2 == -1; var2 = var3.id) {
   ...
   private void cleanupFreeList() throws RecordStoreException {
      RecordStore.RecordHeader var1 = new RecordStore.RecordHeader();
      RecordStore.RecordHeader var2 = new RecordStore.RecordHeader();
      RecordStore.RecordHeader var3 = null;

      try {
         int var4 = this.dbFirstFreeBlockOffset;

         while(var4 != 0) {
            var1.load(var4);
            if (var1.offset >= this.dbDataEnd) {
               if (var1.id != -1) {
                  throw new RecordStoreException("cleanupFreeList id is not -1");
               }

               if (var2.offset == 0) {
                  this.dbFirstFreeBlockOffset = var1.dataLenOrNextFree;
               } else {
                  var2.dataLenOrNextFree = var1.dataLenOrNextFree;
                  var2.store();
               }

               var4 = var1.dataLenOrNextFree;
            } else {
               var4 = var1.dataLenOrNextFree;
               var3 = var2;
               var2 = var1;
