...
public abstract class CustomItem extends Item {
      ...
      if (this.visible) {
         ...
         if (this.exceptionImageItem != null) {
            ...
            try {
               ...
               if (!isEmpty) {
                  int width = this.contentWidth - 2 * CUSTOM_BORDER_PAD;
                  height = h - this.labelHeight - 2 * CUSTOM_BORDER_PAD;
                  if (this.workImage != null && this.workImage.getWidth() == width && this.workImage.getHeight() == height) {
                     this.workImage.getPixmap().nativeSetImageColor(0);
                  } else {
                     this.workImage = new Image(Pixmap.createPixmap(width, height, true, 0));
                  }

                  this.paint(this.workImage.getGraphics(), width, height);
                  g.getImpl().drawPixmap(this.workImage.getPixmap(), g.getTranslateX(), g.getTranslateY());
                  return;
               }
   ...
   boolean isFocusable() {
      try {
         ...
         if (this.itemCommands.length() < 1 && (h == 0 || this.getPrefContentWidth(h) == 0)) {
            return false;
         }
      } catch (Throwable var2) {
         this.handleThrowable(var2);
      }
   ...
   private void handleThrowable(Throwable thr) {
      ...
      if (this.exceptionImageItem == null && this.owner != null) {
         synchronized(Display.LCDUILock) {
            ...
            this.exceptionImageItem = new ImageItem(this.getLabel(), brokenImage, this.layout, (String)null, 1);
   ...
   void displayAlert() {
      Alert customItemAlert = new Alert(this.owner.getTitle(), TextDatabase.getText(1) + "\n\n" + this.textAlert, (Image)null, (AlertType)null);
      customItemAlert.setTimeout(-2);
   ...
   static {
      ...
      brokenImage = new Image(Pixmap.createPixmap(13));
