...
public abstract class Displayable {
   ...
   static final int MAXIMUM_NUMBER_OF_COMMANDS = 30;
   ...
   static final int NOT_LISTENING = 0;
   static final int LISTENING_FOR_PRESS_OR_MOTION = 1;
   static final int LISTENING_FOR_RELEASE_OR_MOVE = 2;
   Ticker ticker;
   String title;
   int[] viewport;
   ...
   boolean layoutValid;
   int pointerListeningState;
   boolean nativeWrapper;
   ...
   public void addCommand(Command cmd) {
      if (cmd == null) {
         ...
         synchronized(Display.LCDUILock) {
            if (this.displayableCommands.length() >= 30) {
               throw new IllegalStateException("Maximum number of commands exceeded!");
            } else {
   ...
   void callPointerDiscontinue(int x, int y) {
   }
   ...
   void setTickerImpl(Ticker newTicker) {
      ...
      if (oldTicker != newTicker) {
         ...
         if (Display.displayActive && this.myDisplay != null && !this.fullScreenMode && this == this.myDisplay.getCurrentImpl()) {
            ...
            if (oldTicker != null) {
               if (newTicker != null) {
                  oldTicker.holdActive();
               } else {
                  oldTicker.hideNotify();
               }
            }

            if (newTicker != null) {
               newTicker.showNotify();
            }
   ...
   void setTitleImpl(String s) {
      this.title = s;
      if (this.myDisplay != null && this.myDisplay.getCurrentImpl() == this) {
   ...
   void callPointerDragged(int keyCode) {
   }
   ...
   void callShowNotify(Display d) {
      ...
      synchronized(Display.LCDUILock) {
         ...
         if (this.pointerListeningState == 0) {
            this.myDisplay.registerForPointerEvents(0);
         }
   ...
   void repaintArea(int x, int y, int width, int height) {
      if (this.displayed && this.myDisplay.getCurrentImpl() == this) {
   ...
   final void updateSoftkeys(boolean createCL) {
      this.updateSoftkeys(createCL, true);
   }

   void updateSoftKeyLabel(String labelText, int index) {
   }

   final void updateSoftkeys(boolean createCL, boolean paintLabels) {
      synchronized(Display.LCDUILock) {
         if (!this.usesSoftLabel()) {
            ...
            if (createCL && this.myDisplay != null && ((Displayable)this.myDisplay.getOptionsMenu()).parentDisplayable == this) {
               optionsMenu = this.myDisplay.getOptionsMenu();
               optionsMenuIndex = optionsMenu.getHighlightedIndex();
            ...
            if (!this.fullScreenMode) {
               softkeyManager.selectSoftkeys(this, this.optionMenuCommands, this.softkeyCommands, optionsMenu != null ? optionsMenu.isShowing() : false);
            }
   ...
   String getInternalTitle() {
      ...
      if (this.title == null || this.title.length() == 0) {
         _title = this.myDisplay.getMIDletName();
      }
   ...
   void clearScreen(Graphics g) {
      if (!this.isNativeDelegate()) {
         if (this.isPopup()) {
            g.clearClipArea(false, true);
         } else if (!(this instanceof GameCanvas) && (!(this instanceof Canvas) || !this.fullScreenMode && UIStyle.isCanvasHasBgImage())) {
            g.clearClipArea(true, false);
         } else {
            g.clearClipArea(false, false);
         }
   ...
   void tickerHidden(boolean holdActive) {
      if (holdActive) {
         this.ticker.holdActive();
      } else {
   ...
   boolean isPowerSavingActive() {
      return this.myDisplay != null ? this.myDisplay.nativeIsPowerSavingEnabled() : false;
   }
