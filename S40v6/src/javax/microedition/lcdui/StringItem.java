...
public class StringItem extends Item {
   ...
   int callPreferredWidth(int h) {
      ...
      if (this.lockedWidth != -1) {
         ...
         if (this.strNLconsidered != null && !isStringEmpty(this.strNLconsidered)) {
            ...
            if (this.hasOffsetTextLines && prefW < DEFAULT_WIDTH) {
   ...
   void callPaint(Graphics g, int width, int height, boolean isFocused) {
      ...
      if (this.strNLconsidered != null && !isStringEmpty(this.strNLconsidered) || this.isFocusable()) {
         ...
         if (!UIStyle.isAlignedLeftToRight) {
            translateX += width - this.contentWidth;
         }
         ...
         if (this.appearanceMode == 1) {
            ...
            if (isFocused) {
               ...
               if (UIStyle.isAlignedLeftToRight) {
                  ...
                  x1 = tX + w - this.lastLineWidth - 1;
               }

               ng.fillRect((short)x1, (short)(tY + this.stringHeight - height - this.lineHeight), (short)(this.lastLineWidth + 2), (short)(this.lineHeight + TextBreaker.DEFAULT_TEXT_LEADING));
         ...
         if (isFocused) {
            if (this.textLines != null && !this.textLines.isEmpty()) {
               ...
               if (UIStyle.isAlignedLeftToRight) {
                  ...
                  x1 = tX + this.currentOffsetWidth - this.lastLineWidth - 1;
               }

               ng.fillRect((short)x1, (short)(tY + height - this.lineHeight), (short)(rectWidth + 2), (short)(this.lineHeight + TextBreaker.DEFAULT_TEXT_LEADING));
            }
   ...
   boolean hasOffsetTextLines(int rowH, int pW, int viewableWidth) {
      this.hasOffsetTextLines = false;
      if (this.itemIndex > 0 && !this.equateNLB() && !this.isButton() && this.getLabel() == null && this.lockedWidth == -1 && !this.shouldHShrink()) {
         ...
         if (mayIncludeItemInBounds) {
            this.callPreferredHeight(-1);
            this.hasOffsetTextLines = this.textLinesOffset.size() > 0;
            this.setToDefaults();
         }
   ...
   private void defineStringItemTextLines(int widthForStringItem, int heightForStringItem, int offsetWidth, int heightOffset) {
      ...
      if (toDoAgain) {
         ...
         if (this.strNLconsidered != null && !isStringEmpty(this.strNLconsidered)) {
            ...
            this.layoutToConsider = tempLayout;
            if (!this.hasOffsetTextLines && (this.currentOffsetWidth == 0 || this.currentOffsetHeight == 0)) {
               this.offsetStringHeight = 0;
               this.currentOffsetHeight = 0;
               this.verticalOffsetStringTranslation = 0;
            } else {
   ...
   private void setFontImpl(Font newFont) {
      ...
      if (newFont != this.font || newFont == null) {
         ...
         this.paintFont = null;
         this.ellipsisWidth = this.font.stringWidth(TextDatabase.getText(34));
   ...
   private void defineNotOffsetTextLines(TextLine tline, TextBreaker breaker, int heightForStringItem) {
      ...
      while(true) {
         do {
            if ((tline = breaker.getTextLine(this.currentWidth)) == null) {
               breaker.setTruncation(false);
               return;
            }

            this.lineHeight = tline.getTextLineHeight();
         } while(this.stringHeight + this.lineHeight > heightForStringItem && heightForStringItem != -1);
