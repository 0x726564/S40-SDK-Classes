...
public class Gauge extends Item {
   ...
   void setMaxValueImpl(int maxValue) {
      ...
      this.maxValue = maxValue;
      if (this.ownerAsAlert != null) {
         if (this.ownerAsAlert.displayed) {
            this.ownerAsAlert.nativeUpdateIndicator(this.value, maxValue);
         }
   ...
   void setValueImpl(int value, boolean hasItemStateChanged) {
      ...
      if (this.value != value) {
         this.value = value;
         action = 1;
      } else if (this.maxValue == -1 && this.value == 3) {
         action = 2;
      }
      ...
      if (this.ownerAsAlert != null) {
         if (this.ownerAsAlert.displayed) {
            if (action == 1) {
               this.ownerAsAlert.nativeUpdateIndicator(value, this.maxValue);
            } else if (action == 2) {
               this.ownerAsAlert.nativeUpdateIndicator(3, -1);
            }
         }
      } else if (this.visible) {
         if (action == 1) {
            if (this.owner instanceof Form && hasItemStateChanged) {
               this.owner.changedItemState(this);
            }

            this.setPixmap();
         } else if (action == 2 && this.gaugePixmap != null) {
   ...
   void callPaint(Graphics g, int w, int h, boolean isFocused) {
      ...
      synchronized(Display.LCDUILock) {
         ...
         if (this.isInteractive) {
            this.renderInteractiveGaugeImage(g, valueGraphicZone);
            ...
            this.renderNonInteractiveGaugeImage(g, isFocused);
         }

      }
   }

   void renderInteractiveGaugeImage(Graphics g, Zone zone) {
      com.nokia.mid.impl.isa.ui.gdi.Graphics ng = g.getImpl();
      ...
      if (UIStyle.isAlignedLeftToRight) {
         ng.drawPixmap(interactiveKnobPixmap, (int)((long)posX + p), posY);
      } else {
         ng.drawPixmap(interactiveKnobPixmap, (int)((long)(posX + width - interactiveNotchPixmap.getWidth()) - p), posY);
      }
      ...
      if (UIStyle.isAlignedLeftToRight) {
         ng.drawTextLine(minValueTextZone.x + posX, minValueTextZone.y + posY + offsetY, minValueTextZone.width, TextBreaker.breakOneLineTextInZone(minValueTextZone, "0"), 1, true);
         ng.drawTextLine(maxValueTextZone.x - posX, minValueTextZone.y + posY + offsetY, minValueTextZone.width, TextBreaker.breakOneLineTextInZone(maxValueTextZone, Integer.toString(this.maxValue)), 3, true);
      } else {
         ng.drawTextLine(minValueTextZone.x + posX, minValueTextZone.y + posY + offsetY, minValueTextZone.width, TextBreaker.breakOneLineTextInZone(minValueTextZone, Integer.toString(this.maxValue)), 1, true);
         ng.drawTextLine(maxValueTextZone.x, maxValueTextZone.y + posY + offsetY, maxValueTextZone.width, TextBreaker.breakOneLineTextInZone(maxValueTextZone, "0"), 3, true);
      }
   ...
   void renderNonInteractiveGaugeImage(Graphics g, boolean isFocused) {
      com.nokia.mid.impl.isa.ui.gdi.Graphics ng = g.getImpl();
      int posX = g.getTranslateX();
      int posY = g.getTranslateY();
      boolean aForm = false;
   ...
   void callShowNotify() {
      ...
      synchronized(Display.LCDUILock) {
         if (this.gaugePixmap == null) {
            this.setValueImpl(this.value, false);
            this.setMaxValueImpl(this.maxValue);
         }
   ...
   void setOwner(Screen owner) {
      ...
      synchronized(Display.LCDUILock) {
         this.ownerAsAlert = null;
         if (owner instanceof Alert) {
            this.ownerAsAlert = (Alert)owner;
         }
   ...
   int callPreferredWidth(int h) {
      return valueGraphicZone.x + valueGraphicZone.width;
   }
   ...
   boolean hasFocus(int pos) {
      ...
      int itemH = this.getHeight();
      return screenH - pos >= itemH && pos >= 0;
   }
   ...
   static {
      ...
      valueGraphicZone = Displayable.uistyle.getZone(21);
      valueSmallTextZone = Displayable.uistyle.getZone(24);
      valueLargeTextZone = Displayable.uistyle.getZone(25);
      minValueTextZone = Displayable.uistyle.getZone(22);
      maxValueTextZone = Displayable.uistyle.getZone(23);
