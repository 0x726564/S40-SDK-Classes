...
public class TextField extends Item {
   ...
   void callLongLeftSoftKeyPressed() {
      if (this.textEditor.isT9modeAllowed()) {
         this.textEditor.longLeftSoftKeyPressed();
         OptionsMenu optionsMenu = this.owner.myDisplay.getOptionsMenu();
         optionsMenu.setKeepRootOptionsMenu(false);
         optionsMenu.dismiss();
         this.owner.myDisplay.setCurrentInternal((Displayable)null, this.owner);
      }
   ...
   void computePreferredHeight() {
      ...
      if (this.lockedHeight != -1) {
         this.lockedHeight = this.preferredHeight + this.getLabelHeight(-1);
      }
   ...
   class TextEditorOwnerImpl implements TextEditorOwner {
      ...
      public void processTextEditorEvent(int eventType, int eventParam) {
         if (TextField.this.owner != null) {
            ...
            if ((eventType & 2048) != 0 && TextField.this.owner instanceof Form) {
               TextField.this.textEditor.reInitialize(true);
            }

            if ((eventType & 4096) != 0) {
               TextField.this.callLongLeftSoftKeyPressed();
            }
