...
public class Form extends Screen {
   ...
   private int scroll;
   private boolean itemHasOffsetTextLines;
   ...
   public Form(String title, Item[] items) {
      ...
      this.scroll = 0;
      this.itemHasOffsetTextLines = false;
   ...
   void layout() {
      ...
      if (this.numOfItems != 0) {
         ...
         for(int index = 0; index < this.numOfItems; ++index) {
            ...
            if (newLine && rowHeight > 0 || this.itemHasOffsetTextLines) {
            ...
            if (!this.itemHasOffsetTextLines) {
      ...
      if (this.traverseIndex == -1) {
         ...
         while(true) {
            if (this.scrollingIndex >= 0 && (isSpacerCaseUp || y1 - (presentLineY + presentLineHeight) <= this.viewport[3] - FORM_MAX_SCROLL || this.view[1] - scrollItem.lineY <= this.viewport[3] - FORM_MAX_SCROLL)) {
               if (!isSpacerCaseUp) {
                  isSpacerCaseUp = this.traverseSpacerUp(presentLineY, true);
                  if (this.scrollingIndex < 0) {
                     return this.returnNotFocusableItemUp(foundItemIndex);
                  }

                  if (!isSpacerCaseUp) {
                     isSpacerCaseUp = this.traverseSpacerUp(this.items[this.scrollingIndex].lineY, false);
      ...
      if (this.traverseIndex == -1) {
         ...
         while(true) {
            if (this.scrollingIndex < this.numOfItems && (isSpacerCase || presentLineY - y1 <= this.viewport[3] - FORM_MAX_SCROLL || scrollItem.lineY + scrollItem.rowHeight <= this.view[1] + 2 * this.viewport[3] - FORM_MAX_SCROLL)) {
               if (!isSpacerCase) {
                  isSpacerCase = this.traverseSpacerDown(presentLineY, true);
                  if (this.scrollingIndex >= this.numOfItems) {
                     return this.returnNotFocusableItemDown(foundItemIndex);
                  }

                  if (!isSpacerCase) {
                     isSpacerCase = this.traverseSpacerDown(this.items[this.scrollingIndex].lineY, false);
   ...
   private boolean traverseSpacerDown(int presentLineY, boolean setScrollingIndex) {
      int index = this.scrollingIndex;

      boolean isSpacerCase;
      Item auxItem;
      for(isSpacerCase = false; index < this.numOfItems && (auxItem = this.items[index]).lineY == presentLineY; ++index) {
         if (!isSpacerCase && auxItem.shouldSkipTraverse() && auxItem.bounds[3] > this.viewport[3] - FORM_MAX_SCROLL) {
            isSpacerCase = true;
         }
      }

      if (setScrollingIndex && index < this.numOfItems) {
   ...
   private boolean traverseSpacerUp(int presentLineY, boolean setScrollingIndex) {
      int index = this.scrollingIndex;

      Item auxItem;
      boolean isSpacerCaseUp;
      for(isSpacerCaseUp = false; index >= 0 && (auxItem = this.items[index]).lineY == presentLineY; --index) {
         if (!isSpacerCaseUp && auxItem.shouldSkipTraverse() && auxItem.bounds[3] > this.viewport[3] - FORM_MAX_SCROLL) {
            isSpacerCaseUp = true;
         }
      }

      if (setScrollingIndex && index >= 0) {
         this.scrollingIndex = index;
      }
   ...
   private void rightToLeftAlignementSwitch() {
      for(int index = 0; !UIStyle.isAlignedLeftToRight && index < this.numOfItems; ++index) {
         ...
         if (tempItem.hasOffsetTextLines) {
   ...
   private int makeNewLine(int rowHeight, int rowStart, int index, int pW) {
      ...
      if (index > 0 && this.itemHasOffsetTextLines) {
   ...
   private void appendStringItemToStringItem(int rowHeight, int rowStart, int index, int pW) {
      ...
      if (prevStringItem.currentOffsetHeight > prevStringItem.offsetStringHeight && prevStringItem.textLinesEmpty) {
         ...
         if (!prevStringItem.hasOffsetTextLines) {
   ...
   private int layoutRowHorizontal(int rowStart, int rowEnd, int hSpace, int rowHeight) {
      ...
      if (hSpace <= 0) {
         ...
         } else if (!UIStyle.isAlignedLeftToRight) {
            while(rowStart <= rowEnd) {
               var10000 = this.items[rowEnd].bounds;
               var10000[0] += hSpace;
               --rowEnd;
   ...
   private boolean isNewLine(int index, int pW, int rowStart, int rowHeight) {
      ...
      if (index == rowStart) {
         ...
         if (index > rowStart && !newL) {
            if (!newAlignemnt) {
               this.itemHasOffsetTextLines = indexedItem.hasOffsetTextLines(rowHeight, pW, this.viewable[2]);
            }

            newL = newAlignemnt || pW > this.viewable[2] - CELL_SPACING || this.itemHasOffsetTextLines;
         }
   ...
   private int formScrollUp(int index) {
      ...
      if (this.isTallRow) {
         if (this.scroll <= this.items[this.traverseIndex].lineY) {
            this.isTallRow = false;
         } else if (!this.items[this.traverseIndex].hasOffsetTextLines) {
            this.isTallRow = true;
            var10000 = this.view;
