...
public class TonePlayer extends MediaPlayer {
   ...
   public void setDataSource(JavaProducerSource source) throws IOException {
      if (this.locator.getCreatedBy() == 3 && !this.locator.isToneDeviceLocator()) {
         String url = this.locator.getBasicLocator();
         InputStream is = null;
         is = Connector.openInputStream(url);
         source = new JavaProducerSource(is);
      }

      if (source != null) {
         source.useActiveSource = false;
      }
   ...
   protected void doPrefetch() throws MediaException {
      if (this.locator.isToneDeviceLocator()) {
         ...
         this.mediaData = nConvertToneSequenceToMIDI(this.dataSource.getData());
         this.dataSource.setData(this.mediaData);
         super.doPrefetch();
      }
