...
public class Protocol implements FileConnection, ConnectionBaseInterface {
   ...
   private static int maxFileHandlesLimit = filesys_MIDletMaxFileHandles();
   private static int currentlyUsedFileHandles = 0;
   ...
   static native int filesys_MIDletMaxFileHandles();
      ...
      FileConnectionOutputStream() throws IOException {
         if (Protocol.this.fileHandle == -1) {
            synchronized(Protocol.lock) {
               if (Protocol.currentlyUsedFileHandles + 1 > Protocol.maxFileHandlesLimit) {
                  throw new IOException("Too many file handles open");
               }

               if (Protocol.this.filesys_open()) {
                  Protocol.currentlyUsedFileHandles++;
               }
            }
      ...
      public void close() throws IOException {
         if (!Protocol.this.out_closed) {
            ...
            if (Protocol.this.in == null && Protocol.this.in_closed) {
               ...
               synchronized(Protocol.lock) {
                  if (Protocol.currentlyUsedFileHandles > 0) {
                     Protocol.currentlyUsedFileHandles--;
                  }
               }
